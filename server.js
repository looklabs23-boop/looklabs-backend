...
app.post('/address', requireAuth, async (req, res) => {
  try {
    ...
    if (existing.rows.length > 0) {
      await pool.query(
        `UPDATE addresses SET full_name=$1, line1=$2, line2=$3, city=$4, state=$5, zip=$6, country=$7, updated_at=NOW()
         WHERE user_id=$8`,
        [full_name, line1, line2, city, state, zip, country || 'US', req.userId]
      );
    } else {
      await pool.query(
        `INSERT INTO addresses (user_id, full_name, line1, line2, city, state, zip, country)
